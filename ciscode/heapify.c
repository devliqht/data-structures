...
void heapify(MinHeap* pq, int i) {
    ...
    while (curr < pq->size) {
    	...
    	int LCidx = 2*curr+1, RCidx = 2*curr+2;
     	if (LCidx < pq->size && pq->heap[LCidx].duration <= pq->heap[curr].duration) smallest = LCidx;
      	if (RCidx < pq->size && pq->heap[RCidx].duration <= pq->heap[smallest].duration) smallest = RCidx;
       
       	if (smallest != curr) {
        	Task temp = pq->heap[curr];
         	pq->heap[curr] = pq->heap[smallest];
