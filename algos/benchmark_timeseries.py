...
# Calculate C factor for O(N^2) algorithms to prevent Python from freezing 
# on 1,000,000 items (which would take ~36 hours to run normally)
test_arr = [random.randint(0, MAX_VAL) for _ in range(2000)]
c_bubble = measure_time(bubble_sort, test_arr) / (2000**2)
c_select = measure_time(selection_sort, test_arr) / (2000**2)
c_insert = measure_time(insertion_sort, test_arr) / (2000**2)

for n in N_values:
    arr = [random.randint(0, MAX_VAL) for _ in range(n)]
    
    # Mathematical extrapolation for O(n^2)
    times["Bubble Sort"].append(c_bubble * (n**2))
    times["Selection Sort"].append(c_select * (n**2))
    times["Insertion Sort"].append(c_insert * (n**2))
    
    # Direct timing measurement for O(N)
